...
function phpscript(){
    $dataArray = db_get_array("SELECT ?:products.*, ?:product_descriptions.product, ?:product_descriptions.full_description 
    FROM ?:products
    LEFT JOIN ?:product_descriptions ON ?:products.product_id = ?:product_descriptions.product_id
    WHERE ?:product_descriptions.lang_code = ?s",'ru');
    foreach($dataArray as $key => $value ){
        $value['product_code']="PR-".$value['product_id'];
        db_query("UPDATE ?:products SET product_code = ?s
        WHERE product_id = '$value[product_id]'",$value['product_code']);
        if($value['product_id']%2==0){
            $value['product']=$value['product'].' '.date('d/m/y');
           db_query("UPDATE ?:product_descriptions SET product = ?s
        WHERE product_id = '$value[product_id]'",$value['product']);
            $value['list_price']=rand(1000,10000);
            db_query("UPDATE ?:products SET list_price = ?i
        WHERE product_id = '$value[product_id]'",$value['list_price']);
        }
        else{
            $value['list_price']=rand(100,500);
            db_query("UPDATE ?:products SET list_price = ?i
        WHERE product_id = '$value[product_id]'",$value['list_price']);
        }
        $value['amount']=rand(1,100);
        db_query("UPDATE ?:products SET amount = ?i
        WHERE product_id ='$value[product_id]'",$value['amount']);
        $value['full_description'] = str_replace("."," END",$value['full_description']);
    }
    uasort($dataArray,'cmp_function');
    $filename = 'array.txt';
    file_put_contents($filename,var_export($dataArray,true));
    fn_print_r($dataArray);
}