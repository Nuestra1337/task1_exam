...
<p action="{""|fn_url}" method="post" name="manage_products_form" id="manage_products_form"
   data-ca-main-content-selector="[data-ca-main-content]" xmlns="http://www.w3.org/1999/html">
...
{/hook}
    {$today = date('d.m.y')}
    <div class="timer" style ="padding-top: 20px;padding-bottom: 20px;text-align: center"> Сегодня <span style="color: blue;size: 14px;font-weight: bold "/>{$today}</span>. До следующей синхронизации осталось: {literal}<script>
    var upgradeTime = 172800;
...
</script>{/literal} <span id ="countdown" class="timer" style="color:red;size:14px;font-weight: bold"></span> </div> <br>
    {$sumOfPrice=0}
    {foreach from=$products item=product}
        {$sumOfPrice = $sumOfPrice + $product.price}
        {/foreach}
    Сумма цен продуктов, расположенных на первой странице = {$sumOfPrice}<br>
    Сумма кол-ва продуктов, расположенных на первой странице = {$sumOfAmount}
